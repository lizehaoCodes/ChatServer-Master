...
{
    /*
    1、将 allgroup表 和 groupuser表 根据 grouopid 进行 内连接查询
    2、根据 userid 在 groupuser表 中查询该用户所在的群组信息（groupid、userid、grouprole）
    3、根据群组信息，查询 groupuser表 的所有 userid，最终通过 user表 进行 多表联合查询，查出用户的详细信息
    */
    char sql[1024] = {0};
    ...
    for (Group &group : groupVec)
    {
        ...
        sprintf(sql, "select a.id,a.name,a.state,b.grouprole from user a inner join groupuser b \
                    on b.userid = a.id where b.groupid=%d",
                group.getId());
